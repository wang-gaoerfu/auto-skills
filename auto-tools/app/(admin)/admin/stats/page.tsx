  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 text-sm mt-1">系统数据统计概览</p>
      </div>
        ...
        <>
          ...
          {stats?.popularTools && stats.popularTools.length > 0 && (
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">热门工具</h3>
              <div className="space-y-3">
                {stats.popularTools.map((tool, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className="text-gray-700">{tool.name}</span>
                    <span className="text-gray-500">{tool.count} 次使用</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Total usage */}
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">总使用次数</h3>
            <p className="text-3xl font-semibold text-gray-900">{stats?.totalUsage || 0}</p>
          </div>
        </>
      )}
    </div>
